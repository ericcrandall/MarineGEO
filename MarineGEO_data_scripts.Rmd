---
title: "MarineGEO Data Formatting and Summarizing"
...
# to do:
# split date fields into year, month, day
# reformat name fields
...
class(fish_samples$catalogNumber)
...
fishmap<-ggmap(dmap) + geom_point(data = fish6, mapping = aes(x = decimalLongitude, y = decimalLatitude, size=count, color=richness)) + scale_color_gradient(low = "green", high="red") + guides(color=guide_colorbar(title="Species Richness",), size=guide_legend(title="Individual Count")) + theme(axis.title=element_blank())
...
class(algae_samples$taxonRank) # Melinda needs to fix this...
...
invert_events$habitatBiotic<-NA #this needs to be aligned with the schema
...
invert_samples$catalogNumber<-NA
invert_samples$otherCatalogNumbers<-NA
...
#ggsave(invert,filename="invert_map2.pdf")
```
...
#ggsave(meiomap,filename="meio_map2.pdf")
```
...
class(arms_events$minimumDepthInMeters)
class(arms_events$recordedBy) #ask Laetitia to follow format
class(arms_events$samplingProtocol) #some missing. ask Freya to fill this in
class(arms_events$habitatGeomorphologicalZone) #some missing. ask Freya to fill this in
class(arms_events$habitatSubstrate) # some missing. ask Freya to fill this in
class(arms_events$habitatBiotic)

#Recommended fields
arms_events$eventRemarks<-NA
class(arms_events$locality)
class(arms_events$year)
class(arms_events$month)
class(arms_events$day)
arms_events$verbatimCoordinates<-NA
...
armsmap<-ggmap(dmap) + geom_point(data = arms5, mapping = aes(x = decimalLongitude, y = decimalLatitude, size=individualCount, color=richness)) + scale_color_gradient(low = "green", high="red") + guides(color=guide_colorbar(title="Type Richness",), size=guide_legend(title="Individual Count")) + theme(axis.title=element_blank()) 

armsmap

#ggsave(almap,filename="arms_map2.pdf")
```
...
species<-sub(pattern=".+_(\\d{8})_(\\w+-[a-z]+)_.+", replacement = "\\2", trans_organism_photos$associatedMedia,perl=T)
...
class(trans_events$minimumDepthInMeters)
class(trans_events$recordedBy) 
class(trans_events$samplingProtocol) #some missing. ask Freya to fill this in
class(trans_events$habitatGeomorphologicalZone) #some missing. ask Freya to fill this in
class(trans_events$habitatSubstrate) # some missing. ask Freya to fill this in
class(trans_events$habitatBiotic)

#Recommended fields
trans_events$eventRemarks<-NA
class(trans_events$locality)
class(trans_events$year)
class(trans_events$month)
class(trans_events$day)
trans_events$verbatimCoordinates<-NA
...
transmap<-ggmap(dmap) + geom_point(data = trans5, mapping = aes(x = decimalLongitude, y = decimalLatitude, size=individualCount, color=richness)) + scale_color_gradient(low = "green", high="red") + guides(color=guide_colorbar(title="Species Richness",), size=guide_legend(title="Individual Count")) + theme(axis.title=element_blank()) 

transmap

#ggsave(almap,filename="trans_map2.pdf")
```
