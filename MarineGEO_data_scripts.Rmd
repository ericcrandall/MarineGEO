...
class(fish_events[,which(sapply(fish_events, is.logical))])<-"character"
...
ggsave(fish_map,filename="fish_map.pdf")
...
```{r Format events}
#change all empty columns from logical to character class
class(algae_events[,which(sapply(algae_events, is.logical))])<-"character"
...
```{r format samples}
#change all empty columns from logical to character class
class(algae_samples[,which(sapply(algae_samples, is.logical))])<-"character"
...
algae_samples$taxonRank<-as.character(algae_samples$taxonRank) # Melinda needs to fix this...
...
ggsave(algae_map,filename="algae_map.pdf")
```
...
#change all empty columns from logical to character class
class(invert_events[,which(sapply(invert_events, is.logical))])<-"character"
...
```{r format samples}
#change all empty columns from logical to character class
class(invert_samples[,which(sapply(invert_samples, is.logical))])<-"character"
...
ggsave(invert_map,filename="invert_map.pdf")
```
...
#change all empty columns from logical to character class
class(meio_events[,which(sapply(meio_events, is.logical))])<-"character"
...
```{r format samples}
#change all empty columns from logical to character class
class(meio_samples[,which(sapply(meio_samples, is.logical))])<-"character"
...
ggsave(meio_map,filename="meio_map.pdf")
```
...
```{r format events}
#change all empty columns from logical to character class
class(arms_events[,which(sapply(arms_events, is.logical))])<-"character"
...
class(arms_events$minimumDepthInMeters)<-"numeric"
...
```{r format samples}
#change all empty columns from logical to character class
class(arms_samples[,which(sapply(arms_samples, is.logical))])<-"character"
...
ggsave(arms_map,filename="arms_map.pdf")
```
...
```{r format events}
#change all empty columns from logical to character class
class(trans_events[,which(sapply(trans_events, is.logical))])<-"character"
...
class(trans_events$minimumDepthInMeters)<-"numeric"
...
```{r format samples}
#change all empty columns from logical to character class
class(trans_samples[,which(sapply(trans_samples, is.logical))])<-"character"
...
ggsave(trans_map,filename="trans_map.pdf")
```
...
write.csv(MarineGEOHI2,"MarineGEOHI_data_1.0.csv")
